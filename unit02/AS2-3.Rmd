...
#缺項是數字是1，沒有缺項的是0，所以數字越高的代表缺項越多
# 因為 Citizen的標準差最大，所以代表缺項值跟 Citizenship的關係越高
```
...
```{r}
A = merge(A, CountryMap, by.x="CountryOfBirthCode", by.y="Code", all.x=TRUE) #all.x=TRUE)保留所有欄位
```
...
#Vallejo-Fairfield, CA、 San Jose-Sunnyvale-Santa Clara, CA 、San Francisco-Oakland-Fremont, CA、Honolulu, HI 
```
...
#Phoenix-Mesa-Scottsdale, AZ跟St. Cloud, MN
```
